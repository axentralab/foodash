        ...
        {STEPS.map((step,i)=>{
          ...
          return (
            <div key={step.key} className="flex flex-col items-center gap-2 relative z-10">
              <div className={cn("w-10 h-10 rounded-full flex items-center justify-center text-base border-2 transition-all",
      ...
      <div className="p-5 cursor-pointer" onClick={()=>setExpanded(!expanded)}>
        ...
        <div className="flex items-center justify-between mt-3">
          <div className="flex items-center -space-x-2">
            ...
            {order.items.length>3&&<div className="w-9 h-9 rounded-xl bg-gray-100 border-2 border-white flex items-center justify-center text-xs font-bold text-gray-400">+{order.items.length-3}</div>}
          </div>
        ...
        <div className="px-5 pb-5 border-t border-gray-100 pt-5 space-y-5">
          ...
          <div className="flex gap-3">
            {order.status==="delivered"&&(
              <Link href="/dashboard/menu" className="btn-primary flex-1 flex items-center justify-center gap-2 py-3.5 text-sm">
                🔄 Reorder
              </Link>
            )}
            {canCancel&&(
              <button onClick={handleCancel} disabled={cancelling}
                className="flex-1 py-3.5 rounded-2xl text-sm font-semibold border border-red-200 text-red-500 hover:bg-red-50 transition-colors disabled:opacity-50">
...
export default function OrdersPage() {
  ...
  if (loading) {
    return (
      <div className="p-5 lg:p-10 max-w-3xl mx-auto space-y-4">
        <div className="h-8 bg-gray-200 rounded-xl w-40 animate-pulse mb-6"/>
    ...
    <div className="min-h-full p-5 lg:p-10 max-w-3xl mx-auto pb-28">
      ...
      {active.length>0&&(
        <section className="mb-8">
          ...
          <div className="space-y-4">{active.map(o=><OrderCard key={o.id} order={o} onCancel={handleCancel}/>)}</div>
        </section>
      )}
      ...
      {past.length>0&&(
        <section>
          ...
          <div className="space-y-4">{past.map(o=><OrderCard key={o.id} order={o} onCancel={handleCancel}/>)}</div>
        </section>
      )}
      ...
      {orders.length===0&&(
        <div className="text-center py-20">
          ...
          <h3 className="font-bold text-dark-100 text-xl mb-2">No orders yet</h3>
          <p className="text-gray-400 mb-6">Your order history will appear here</p>
          <Link href="/dashboard/menu" className="btn-primary inline-flex items-center gap-2 px-8 py-3">Start Ordering</Link>
        </div>
      )}
    </div>
  );
